...
import { RouteTimeline } from '@/app/components/route-detail/RouteTimeline';
...
// RouteTimeline 렌더링에 필요한 legs 구조가 갖춰져 있는지 확인
function hasValidLegsForTimeline(legs: any[]): boolean {
  if (!legs || legs.length === 0) return false;
  return legs.every((leg: any) => leg.start?.name !== undefined && leg.end?.name !== undefined);
}
...
export function StatsPage({ onBack, onNavigate, onOpenDashboard, onOpenSubway }: StatsPageProps) {
  ...
  const handleNavigateToRoute = () => {
    const pair = routePairs.find(
      (p) =>
        p.departure_name === selectedPair?.departure && p.arrival_name === selectedPair?.arrival
    );
    if (pair) {
      setDepartureArrival(
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        setLoading(true);
        const detail = await getRouteStats(selectedPair.departure, selectedPair.arrival);
        setStats(detail);
      } catch (err) {
      ...
      >
        ...
        {error && !loading && (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            <p className="text-red-500 font-['Noto_Sans_KR',sans-serif] text-[14px]">{error}</p>
            <button
        ...
        {!loading && !error && routePairs.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            <span className="text-[48px]">{'\u{1F3C1}'}</span>
          ...
          <>
            ...
            <div className="flex items-center justify-between p-4 bg-white rounded-2xl shadow-sm mb-4">
              <div className="flex items-center gap-1.5 min-w-0">
                <span className="text-[8px] text-[#4CAF50] flex-shrink-0">{'\u{25CF}'}</span>
                <span className="text-[13px] font-semibold text-gray-800 font-['Noto_Sans_KR',sans-serif] truncate">
                  {stats.departure_name}
                </span>
                <span className="text-gray-400 text-[14px] flex-shrink-0 mx-1">{'\u{2192}'}</span>
                <span className="text-[8px] text-[#F44336] flex-shrink-0">{'\u{25CF}'}</span>
                <span className="text-[13px] font-semibold text-gray-800 font-['Noto_Sans_KR',sans-serif] truncate">
                  {stats.arrival_name}
                </span>
              </div>
                ...
                >
                  ...
                  <span
                    className={`text-[13px] font-semibold font-['Noto_Sans_KR',sans-serif] ${
                      activeTimeSlot === key ? 'text-[#2E7D32]' : 'text-gray-500'
                    }`}
                  >
                    {slot.label}
                  </span>
            ...
            {/* 해당 시간대 데이터 없음 */}
            {currentSlot && (!currentSlot.routes || currentSlot.routes.length === 0) && (
              <div className="bg-white rounded-2xl p-8 shadow-sm text-center mb-4">
                <span className="text-[36px]">{'\u{1F4CA}'}</span>
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-around">
                      ...
                      <div className="flex flex-col items-center gap-0.5">
                        <span className="text-[10px] text-gray-400">{'평균 시간'}</span>
                        <span className="text-[15px] font-bold text-[#4CAF50] whitespace-nowrap">
                          {formatDuration(bestRoute.avg_duration)}
                        </span>
                      </div>
                      <div className="flex flex-col items-center gap-0.5">
                        <span className="text-[10px] text-gray-400">{'경주'}</span>
                        <span className="text-[15px] font-bold text-[#4CAF50]">
                          {bestRoute.race_count}
                          {'회'}
                        </span>
                      </div>
                      <div className="flex flex-col items-center gap-0.5">
                        <span className="text-[10px] text-gray-400">{'승리'}</span>
                        <span className="text-[15px] font-bold text-[#4CAF50]">
                          {bestRoute.wins}
                          {'승'}
                        </span>
            ...
            {currentSlot && currentSlot.routes && currentSlot.routes.length > 0 && (
              <div className="bg-white rounded-2xl p-4 shadow-sm mb-4">
                ...
                <div className="flex flex-col gap-3">
                  {currentSlot.routes.map((route: RouteWinRateInfo, index: number) => {
                    const isExpanded = expandedRoute === route.route_label;
                    return (
                      <div key={route.route_label}>
                        <div
                          onClick={() => setExpandedRoute(isExpanded ? null : route.route_label)}
                          className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl cursor-pointer active:bg-gray-100 transition-colors"
                        >
                            ...
                            <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  ...
                                  backgroundColor: getWinRateColor(route.win_rate),
                                }}
                              />
                            </div>
                          </div>
                        ...
                        {isExpanded && route.leg_detail && (
                          <div className="mt-2 ml-2 mr-1 p-3 bg-white rounded-xl border border-gray-100">
                            {hasValidLegsForTimeline(route.leg_detail.legs) ? (
                              <RouteTimeline
                                legs={route.leg_detail.legs}
                                isLoading={false}
                                totalTime={route.leg_detail.total_time}
                                totalDistance={route.leg_detail.total_distance}
                                totalWalkTime={route.leg_detail.total_walk_time}
                                totalWalkDistance={route.leg_detail.total_walk_distance}
                                transferCount={route.leg_detail.transfer_count}
                                pathType={route.leg_detail.path_type}
                              />
                            ) : (
                              <p className="text-[12px] text-gray-400 text-center py-4 font-['Noto_Sans_KR',sans-serif]">
                                상세 경로 정보가 없습니다.
                              </p>
                            )}
                          </div>
            ...
            {/* 인사이트 */}
            {stats.insight && stats.insight !== '아직 충분한 데이터가 없습니다.' && (
              <div className="bg-white rounded-xl p-4 mb-4 shadow-sm">
                <div className="flex items-center gap-3">
                  <span className="text-[22px]">{'\u{1F4A1}'}</span>
