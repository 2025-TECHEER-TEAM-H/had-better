...
import { CharacterMovingTest } from './components/CharacterMovingTest';
...
// ========== 테스트 모드 설정 ==========
// true로 변경하면 캐릭터 이동 테스트 화면으로 전환
const CHARACTER_MOVING_TEST_MODE = true;

export default function App() {
  // 테스트 모드일 경우 테스트 컴포넌트 렌더링
  if (CHARACTER_MOVING_TEST_MODE) {
    return <CharacterMovingTest />;
  }
  ...
  const [pickPlaceCategory, setPickPlaceCategory] = useState<
    'home' | 'work' | 'school' | undefined
  >(undefined);
        ...
        <div className="absolute inset-0 z-20 pointer-events-none">
          ...
          {!isMapVisible && (
            <div className="absolute inset-0 bg-white pointer-events-auto z-[999]">
              ...
              {currentPage === 'login' && (
                <LoginPage onLogin={handleLogin} onSignUp={handleGoToSignUp} />
              )}
              {currentPage === 'signup' && (
                <SignUpPage onSignUp={handleSignUp} onBack={handleBackToLogin} />
              )}
              {currentPage === 'result' && (
                <GameResultPage onContinue={handleContinue} onNavigate={handleNavigate} />
              )}
              {currentPage === 'dashboard' && <DashboardPage onNavigate={handleNavigate} />}
              {currentPage === 'place-info' && (
                <PlaceInfoPage
          ...
          {isMapVisible && (
            <>
              ...
              {currentPage === 'route-detail' && (
                <RouteDetailPage onNavigate={handleNavigate} routeSelection={routeSelection} />
              )}
              {currentPage === 'place-map' && (
                <PlaceMapPage
                  onNavigate={handleNavigate}
                  place={selectedPlace}
                  fromFavorites={fromFavorites}
