...
class UserBusMonitor:
    ...
    def initialize(self) -> bool:
        ...
        logger.info(f"[UserBusMonitor] Walk Time: {accumulated_seconds}s, ETA: {self.arrival_eta}")
        ...
        for st in stations:
            ...
            if st_name == self.start_station_name or (st_name and st_name.replace(" ", "") == self.start_station_name.replace(" ", "")):
                target_station = st
                break
        ...
        self.station_id = target_station.get('station') or target_station.get('stationId') or target_station.get('stId')
        seq = target_station.get('seq') or target_station.get('staOrder') or target_station.get('stationSeq')
    ...
    def check_arrival(self) -> Optional[Dict[str, Any]]:
        ...
        if self.last_call_time and (now - self.last_call_time).total_seconds() < self.call_interval:
            # 아직 10초 안 지남 (Skip API call)
            return None

        # 3. 실제 API 호출
        self.last_call_time = now
        logger.info(f"[UserBusMonitor] Calling Bus API (Window: -3m ~ +1m, Interval: 10s)")
        
        arrival_info = bus_api_client.get_arrival_info(
            self.station_id, 
            self.public_route_id, 
            self.station_ord
        )
