...
def validate_route_segments(route_leg) -> tuple[bool, str]:
    ...
    """
    segments = RouteSegment.objects.filter(route_leg=route_leg).order_by(
        "segment_index"
    )
    ...
    for seg in segments:
        ...
        if seg.mode in ["BUS", "SUBWAY"]:
            ...
            if seg.distance < 500:
                ...
                return (
                    False,
                    f"{seg.mode} 구간이 너무 짧습니다 ({seg.distance}m). 다른 경로를 선택해주세요.",
                )
            ...
            if route_leg.raw_data and "legs" in route_leg.raw_data:
                ...
                if len(pass_stops) <= 2:
                    ...
                    return (
                        False,
                        f"{seg.mode} 경로의 정류장/역 정보가 부족합니다. 다른 경로를 선택해주세요.",
                    )
    ...
    def post(self, request):
        ...
        try:
            ...
            return success_response(
                data={"error": str(e)}, status=status.HTTP_502_BAD_GATEWAY
            )
        ...
        with transaction.atomic():
            ...
            legs = []
            itineraries = (
                tmap_response.get("metaData", {}).get("plan", {}).get("itineraries", [])
            )
            ...
            for leg_index, itinerary in enumerate(itineraries):
                ...
                for seg_index, segment in enumerate(segments):
                    ...
                    if segment.get("mode") == "WALK":
                        ...
                        linestring_str = segment.get("passShape", {}).get(
                            "linestring", ""
                        )
        ...
        # 응답 생성
        request_parameters = tmap_response.get("metaData", {}).get(
            "requestParameters", {}
        )
...
class SearchItineraryHistoryListView(APIView):
    ...
    def delete(self, request):
        ...
        return success_response(
            data={"deleted_count": deleted_count}, status=status.HTTP_200_OK
        )
...
class SearchItineraryHistoryDetailView(APIView):
    ...
    def get(self, request, search_itinerary_history_id):
        try:
            search_history = (
                SearchItineraryHistory.objects.select_related("route_itinerary")
                .prefetch_related("route_itinerary__legs")
                .get(
    ...
    def delete(self, request, search_itinerary_history_id):
        ...
        return success_response(
            data={"id": search_history.id}, status=status.HTTP_200_OK
        )
...
class RouteLegDetailView(APIView):
    ...
    def get(self, request, route_leg_id):
        try:
            ...
            route_leg = RouteLeg.objects.select_related("route_itinerary").get(
                id=route_leg_id, deleted_at__isnull=True
            )
        ...
        return success_response(
            data=RouteLegDetailSerializer(route_leg).data, status=status.HTTP_200_OK
        )
